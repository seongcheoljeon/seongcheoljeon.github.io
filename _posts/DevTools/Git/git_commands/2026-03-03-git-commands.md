...
`git`은 다양한 명령어를 지원한다. 구글에서 `git cheat sheet` 등의 검색어로 검색하면 많이 찾을 수 있다. 

<https://git-scm.com/docs>

`git`은 파일을 **세 가지 작업 영역으로 관리**한다. 이러한 가상의 공간 구분을 염두에 두고 살펴보면 몇몇 명령어들의 의미가 더욱 명확해진다.

1. Working Directory : 저장소가 추적 중인 파일들이 위치하는 영역이다.
2. Staging Area : 커밋할 준비가 된 (staged) 파일들이 위치하는 영역이다.
3. Repository : 커밋되어 버전을 관리하는 파일들이 위치하는 영역이다. 이 영역의 파일이 수정되면 Working Directory 영역으로 이동된다.
...
| 명령어 | 설명 |
|:---|:---|
| `git config --global user.name "<이름>"` | 커밋에 기록될 사용자 이름 설정 |
| `git config --global user.email "<이메일>"` | 커밋에 기록될 이메일 설정 (GitHub 이메일과 동일하게 권장) |
| `git config --global --add merge.ff false` | `merge` 시 항상 `--no-ff` 적용 (merge commit 강제 생성) |
...
| `git init <저장소 이름>` | 지정한 이름으로 새 저장소 생성 |
...
| 명령어 | 설명 |
|:---|:---|
| `git status` | 현재 저장소 상태 확인 (추적 여부, 변경 파일 등) |
| `git diff` | 마지막 커밋과 현재 변경 내용 비교 |
| `git diff <브랜치> ... <다른 브랜치>` | 두 브랜치 간의 차이점 비교 |
| `git add <파일 이름>` | 파일을 Staging 영역에 추가 |
| `git reset <파일 이름>` | Staging 영역에서 파일 제거 (변경 내역은 유지) |
| `git commit -m "<메시지>"` | Staged 파일을 메시지와 함께 커밋 |
| `git commit -a` | 추적 중인 변경 파일 전체를 커밋 (`-m` 옵션으로 메시지 추가 가능) |
| `git commit --amend` | 마지막 커밋을 수정 (내용 추가 또는 메시지 변경) |
| `git checkout HEAD -- <파일 이름>` | 아직 커밋하지 않은 변경 내역 취소 |
...
| 명령어 | 설명 |
|:---|:---|
| `git branch` | 브랜치 목록 확인 (`*` 표시가 현재 브랜치) |
| `git branch <이름>` | 새 브랜치 생성 |
| `git branch -d <이름>` | 브랜치 삭제 |
| `git checkout <브랜치 이름>` | 작업 브랜치 변경 |
| `git checkout -b <브랜치 이름>` | 브랜치 생성과 동시에 이동 |
| `git merge <브랜치 이름>` | 현재 브랜치에 대상 브랜치를 병합 |
| `git rebase <브랜치 이름>` | 현재 브랜치를 대상 브랜치 위로 재배치 (이력 선형화) |
| `git rebase -i` | 커밋 히스토리를 인터랙티브하게 편집 (squash 등) |
...
| 명령어 | 설명 |
|:---|:---|
| `git rm <파일 이름>` | 저장소 추적 해제 + 로컬 파일 삭제 (삭제 이력 남음) |
| `git rm --cached <파일 이름>` | 저장소 추적만 해제 (로컬 파일 유지) |
| `git mv <파일 이름> <변경될 이름>` | 파일 이름 또는 경로 변경 (변경 이력 남음) |
...
| 명령어 | 설명 |
|:---|:---|
| `git stash` | 현재 변경 내역을 임시 공간에 저장 |
| `git stash pop` | 마지막으로 저장된 stash를 현재 브랜치에 적용 |
| `git stash list` | 저장된 stash 목록 출력 |
| `git stash drop` | 마지막으로 저장된 stash 삭제 |
...
| 명령어 | 설명 |
|:---|:---|
| `git tag` | 커밋에 알기 쉬운 이름을 붙여 참조하기 쉽게 만듦 |
...
| 명령어 | 설명 |
|:---|:---|
| `git log` | 현재 브랜치의 커밋 히스토리 출력 |
| `git log --follow <파일 이름>` | 파일 이름 변경 이력을 포함한 변경 내역 출력 |
| `git show <커밋>` | 특정 커밋의 메타데이터와 변경 내역 출력 |
...
| 명령어 | 설명 |
|:---|:---|
| `git revert <커밋>` | 특정 커밋의 내용을 되돌리는 **새 커밋** 생성 (이력 유지) |
| `git reset <커밋>` | 대상 커밋 이후의 커밋 취소, 변경 내역은 로컬에 보존 |
| `git reset --hard <커밋>` | 대상 커밋 이후의 커밋과 변경 내역 모두 삭제 ⚠️ |

> **`revert` vs `reset`**
> - `revert` : 이력을 **남기면서** 되돌림 → 협업 환경에서 권장
> - `reset` : 이력을 **지우면서** 되돌림 → 로컬 작업에서만 사용 권장
...
| `git remote remove <원격저장소 별칭>` | 원격 저장소 연결 해제 |
| `git fetch <원격저장소 이름>` | 원격 저장소의 변경 내역을 로컬로 가져옴 (병합 없음) |
| `git merge <원격저장소/브랜치>` | 원격 브랜치를 현재 브랜치에 병합 |
| `git push <원격저장소 이름> <브랜치>` | 로컬 커밋을 원격 저장소에 업로드 |
| `git pull <원격저장소 이름>` | `fetch` + `merge` 를 한 번에 실행 |

> **`fetch` vs `pull`**
> - `fetch` : 가져오기만 하고 병합은 하지 않음 → 변경 내역 확인 후 수동 병합
> - `pull` : 가져오기 + 병합을 자동으로 수행
