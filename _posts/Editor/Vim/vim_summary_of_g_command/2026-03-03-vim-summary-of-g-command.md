...
> 💡 **`0` / `^` / `$` vs `g0` / `g^` / `g$` 차이:**  
> `0`, `^`, `$`는 **논리적 줄**(파일 기준) 단위로 동작한다.  
> `g0`, `g^`, `g$`는 **화면 줄**(wrap된 줄) 단위로 동작한다.  
> `wrap`이 꺼져 있거나 줄이 화면 폭 안에 들어오면 동작이 동일하다.  
...
붙여넣은 직후 이어서 입력해야 할 때 `gp`/`gP`가 편하다.
